...
public class CS2ServiceDispatcher {
    ...
    public void enQueue(Frame frames, MessageParser messageParser) {
    ...
    private static class DispatcherTask implements Runnable {
        ...
        public void run() {
            ...
            synchronized (messageQueue) {
                ArrayList<SessionDetails> sessionFramePairs = new ArrayList<>(messageQueue.size());
                ...
                for (SessionDetails details : sessionFramePairs) {
                    ...
                    String userSession;
                    try {
                        userSession = jacksonObjectWrapper.writeValueAsString(playerSession);
                    } catch (Exception e) {
                        log.error("Failed to serialize player session", e);
                        continue;
                    }
